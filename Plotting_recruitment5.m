...
for p=1:5
    ...
    for i=1:N
        ...
        SK_length=25;
        for number=1:length(crisis{p})
            init=min(index(find(index(:,number)),number));
            first_init=init-2*crisis_time;
            if isempty(init)~=1 && init>0 && index(i,number)>0
               init=init-2*crisis_time;
               if init<0
                   init=1;end
               finish1=init+SK_length;
               finish2=finish1+SK_length;
               k=1;
               while k<crisis_time*4;
                     ...
                     finish1=init+SK_length;
                     finish2=finish1+SK_length;
                     if finish2>=length(prueba) 
                         k=crisis_time*4+1; end
                        k=k+1;
               end
               auxiliar=1./reshape(probability(number,i,:),[length(probability(number,i,:)),1]);
               [pks locs]=findpeaks(auxiliar);
               positions=first_init-2*crisis_time+1+SK_length:first_init+crisis_time*2+SK_length-1;
               val=max(pks(find(positions(locs)>first_init+crisis_time-75 & positions(locs)<first_init+crisis_time)));
               mark=positions(locs(find(pks==val,1)));
               index2(i,number)=mark+3*crisis_time;
    ...
    clear index index2 binary_signal binary_signal_2
end