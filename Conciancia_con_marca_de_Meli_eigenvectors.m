    ...
    for k=1:length(binary_signal)
        for j=1:size(binary_signal_2{k},1)
            if isempty(binary_signal_2{k})~=1 && isempty(binary_signal{k})~=1       
                ...
                if length(find(tf))>crisis_time+crisis_time*0.2 || length(find(tf))==length(s_real)
    ...
    for number=1:length(crisis{p})
        if isempty(onset_electrodes(number))~=1
            ...
            for j=1:length(onset_electrodes{number})
                ...
                if start>0
                    value2(counter2,:,:)=abs(V_crisis_prueba(onset_electrodes{number}(j),:,start:final));
                    counter2=counter2+1;
                    detection=1;
                end
            end
...
for j=1:5
    ...
    for i=1:length(new_AOC)
        plot(time,reshape(SzO_mean(new_index(i),j,:),[1 size(SzO_mean,3)]),'color',color(i,:))
        hold on
    end
end
figure(2)
...
for l=1:5
    ...
    for k=1:size(SzO_mean,3)
       [r m p]=regression(AOC(index),SzO_mean(index,l,k)');
       all_reg(k,l)=r;
       %for j=1:1000
        %  [r1 m p]=regression(AOC(index(randperm(length(index)))),SzO_mean(index,l,k)');
         % test(j)=r1;
       %end
      %test=sort(abs(test));
      %if abs(r)>test(950)
    ...
    plot(time, all_reg(:,l),'Linewidth',2)
    hold on
    %hold on
    %index2=find(all_reg_sig(:)>0);
    %scatter(time(index2),all_reg(index2,l));
end
