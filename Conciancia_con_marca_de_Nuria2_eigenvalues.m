...
for p=1:5
    ...
    for number=1:length(crisis{p})
        if isempty(onset_electrodes(number))~=1
            ...
            if start>0
                value2=abs(all_eigenvalues(onset_electrodes{number},start:final));
...
for i=1:length(new_AOC)
    plot(time,SzO_mean(new_index(i),:),'color',color(i,:))
    hold on
end
figure(3)
[new_AOC new_index]=sort(AOC(find(AOC>=0)));
color=colormap(jet(9));
for i=1:length(color)
    x=find(new_AOC==i);
    plot(time,mean(SzO_mean(x,:),1),'color',color(i,:))
    hold on
end
